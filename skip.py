...
for index, anime_entry in enumerate(data, start=1):
    ...
    print(f"\n[{index}/{total_anime}] Processing anime: '{anime_name}' (MAL Code: {mal_code})")
    ...
    # If episodes is an empty list or indicates no episodes, skip this anime.
    ...
    # Try to find the anime in the database (if mal_code is provided)
    anime = None
    if mal_code:
        anime = Anime.query.filter_by(mal_code=mal_code).first()

    # If not found, try fetching from the MAL API (if mal_code exists)
    if not anime:
        if mal_code:
            print(f"Fetching MAL data for '{anime_name}' with MAL Code {mal_code}...")
            response = requests.get(f"https://api.jikan.moe/v4/anime/{mal_code}")
            if response.status_code == 200:
                api_data = response.json().get("data", {})
                if not api_data:
                    print(f"No MAL data found for code {mal_code}. Skipping anime '{anime_name}'.")
                    skipped_animes.append(anime_entry)
                    continue
                anime = Anime(
                    mal_code=mal_code,
                ...
                print(f"Error fetching MAL data for code {mal_code}. Creating minimal entry for '{anime_name}'.")
                anime = Anime(
                    mal_code=None,
                    title=anime_name,
                    poster_image=None
